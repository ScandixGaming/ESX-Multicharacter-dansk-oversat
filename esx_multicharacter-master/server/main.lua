if not ESX then
	error('\n^1Kan ikke starte multikarakteren - du skal bruge ESX Legacy^0')
elseif ESX.GetConfig().Multichar == true then
	local DATABASE do
		local connectionString = GetConvar('mysql_connection_string', '');
		if connectionString == '' then
			error(connectionString..'\n^1Kan ikke starte Multikarakteren - Kan ikke vælge databasen fra mysql_connection_string^0', 0)
	...
	AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
		...
		if identifier then
			if ESX.Players[identifier] then
				deferrals.done(('En spiller er allerede forbundet til serveren med dette spiller-id.\nDit spiller-id: %s:%s'):format(PRIMARY_IDENTIFIER, identifier))
			else
				deferrals.done()
			end
		else
			deferrals.done(('Kan ikke hente spiller-id.\nSpiller-id type: %s'):format(PRIMARY_IDENTIFIER))
		end
	end)
	...
	local function DeleteCharacter(source, charid)
		...
		MySQL.transaction(queries, function(result)
			if result then
				print(('[^2INFO^7] Spiller [%s] %s har slette en karakter (%s)'):format(GetPlayerName(source), source, identifier))
				Citizen.Wait(50)
				SetupCharacters(source)
			else
				error('\n^1Du fejlede at slette din karakter '..identifier..'^0 prøv igen')
			end
		end)
	end
	...
	assert(nil, '^3WARNING: Multikarakteren er deaktiveret - please tjek dine ESX configurationer^0')
end
